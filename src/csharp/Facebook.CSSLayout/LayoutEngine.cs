	...
	public class LayoutEngine
	{
		...
		static void layoutNodeImpl(CSSNode node, float parentMaxWidth)
		{
			...
			setLayoutPosition(node,
				getLeading(mainAxis),
				getLayoutPosition(node, getLeading(mainAxis)) + getMargin(node, getLeading(mainAxis)) +
				getRelativePosition(node, mainAxis));
			setLayoutPosition(node,
				getLeading(crossAxis),
				getLayoutPosition(node, getLeading(crossAxis)) + getMargin(node, getLeading(crossAxis)) +
			...
			while (endLine < node.getChildCount())
			{
				...
				for (int i = startLine; i < endLine; ++i)
				{
						...
						isPosDefined(child, getLeading(mainAxis)))
					{
						...
						// (and margin/border).
						setLayoutPosition(child,
							getPos(mainAxis),
							getPosition(child, getLeading(mainAxis)) +
				...
				for (int i = startLine; i < endLine; ++i)
				{
						...
						isPosDefined(child, getLeading(crossAxis)))
					{
						...
						setLayoutPosition(child,
							getPos(crossAxis),
							getPosition(child, getLeading(crossAxis)) +
					...
					else
					{
						...
						setLayoutPosition(child,
							getPos(crossAxis),
							getLayoutPosition(child, getPos(crossAxis)) + linesCrossDim + leadingCrossDim);
					}
				}
